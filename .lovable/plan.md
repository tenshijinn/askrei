...
Adds a monthly, self-resetting top-10 referrer leaderboard using the pot split from the Refer-To-Earn article, plus a month-end confirmation (in-app snapshot page + email) listing the winning wallets.
...
4. **Confirmation** — on the 1st of each month the closed month is snapshotted and an email is sent to the admin address with the 10 wallets, points and pot percentages (plus a copyable list). The same snapshot is viewable in an admin-only view of the leaderboard card.
...
- `close-referral-month` (verify_jwt=false, internal key required): computes the previous month's top 10, writes snapshot rows idempotently, logs to `ops_events` via `log_ops_event`, and sends the confirmation email. Safe to re-run.
- Cron: `pg_cron` job at 00:15 UTC on day 1 calling `close-referral-month`.

**Email**
- Uses the project email setup. There is currently no email domain/sender configured, so I will open the email setup dialog for `rei.chat` as part of this work; the snapshot and in-app confirmation work regardless, and the email activates once DNS verifies. Recipient address will be confirmed at setup time.
